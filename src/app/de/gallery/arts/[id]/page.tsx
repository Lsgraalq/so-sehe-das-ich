...
async function  ArtPage({ params }: Props) {
    ...
    if (!snapshot.exists()) {
        return <div>Gemälde nicht gefunden</div>
    }
    ...
    <div className="pt-25 flex flex-col gap-10">
        <div className="max-w-[80%]  md:max-w-[70%] xl:max-w-[60%]  mx-auto  relative">
            ...
            <img src="/images/artinfomazok2.png" alt="Pinselstrich" className='absolute left-[-20]  top-[-15] max-w-[15%] md:left-[-40] md:top-[-40] xl:max-w-[15%] xl:top-[-30] xl:left-[-70]' />
...
xl:max-w-[15%] xl:bottom-[-30] xl:right-[-70]'/>
        ...
        <div className="text-xl flex flex-col gap-3 sm:min-w-[80%] sm:mx-auto min-w-[80%] mx-auto md:tetx-xl xl:text-xl">
            ...
            <p className="">Autor: <a href={"/de/artist/" + art.authorId} className="underline">{Author.username}</a></p>
            <span className="italic text-[#B3ABBE]">{art.description}</span>
            <h6 className="">Material: <p className="p-1 bg-[#9773BD] inline-block rounded-sm">{art.canvasType}</p> </h6>
            <h6 className="">Farben: <p className="p-1 bg-[#E24C4C] inline-block rounded-sm">{art.paints?.join(" ")}</p></h6>
            {art.price && (
                <p className="">Preis: {art.price} €</p>
            )}
            <h6 className="italic font-semibold">Ausstellung: ICH SEHE DAS ICH 1</h6>
            <h6 className="">Teilnahme an Auktion: {art.auction}</h6>
            <h6 className="">Größe: <p className="p-1 bg-green-600 inline-block rounded-sm">{art.height}cm</p>  x <p className="p-1 bg-blue-600 inline-block rounded-sm">{art.width}cm</p></h6>
        </div>
    </div>
    <FooterDe></FooterDe>
    </>
